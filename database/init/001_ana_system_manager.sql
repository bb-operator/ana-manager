...
('agent_request_handoff', 'Agent request means immediate handoff', 'Solicitud de agente significa handoff inmediato', 10, 'critical', '{"phrases":["call me","agent","asap","showing","appointment","today","tomorrow"]}', ARRAY['handoff','mark_qualified','notify_slack','stop_cadence']::asm_rule_action[], 'Do not keep asking budget after an agent request.', 'No seguir pidiendo presupuesto despues de solicitud de agente.'),
('frustration_human_review', 'Frustration stops Ana', 'Frustracion detiene a Ana', 20, 'critical', '{"signals":["idiot","stop asking","another agent","angry","upset"]}', ARRAY['human_review','block_reply','stop_cadence','notify_slack']::asm_rule_action[], 'Hostile/frustrated leads go to human review.', 'Leads molestos pasan a revision humana.'),
...
('respect_no_call', 'Respect no-call preference', 'Respetar preferencia sin llamada', 50, 'high', '{"phrases":["no call","dont call","do not call","text only","email only"]}', ARRAY['block_reply']::asm_rule_action[], 'Do not ask for a call after no-call preference.', 'No pedir llamada si el lead dijo que no quiere llamada.');
